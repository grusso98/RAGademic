...
parser = argparse.ArgumentParser(description="Run the RAG-based university notes query system.")
parser.add_argument("--openai", action="store_true", help="Use OpenAI API for LLM and embeddings", required=True)
...
def chunk_text(text: str, chunk_size: int = 1000, overlap: int = 200) -> List[str]:
    ...
    Args:
        ...
        overlap (int, optional): The number of overlapping characters between consecutive chunks. Defaults to 200.
...
def load_notes() -> None:
    ...
    This function iterates through all categories and files in the knowledge base folder,
    extracts the text from PDF documents, chunks the content, and adds it to the vector database.
...
def query_rag(query: str, history: Optional[List[str]] = None) -> str:
    ...
    Args:
        ...
        history (Optional[List[str]], optional): A list of previous interactions (defaults to None).
    ...
    context = "\n\n".join([" ".join(doc) if isinstance(doc, list) else doc for doc in results.get("documents", [])])
    ...
    response = openai.chat.completions.create(
        model=model_name,
        messages=[{"role": "system", "content": "You are a university tutor."},
...
def visualize_embeddings_interactive() -> go.Figure:
    """
    Visualizes document embeddings interactively using Plotly and colors them based on their categories.
    ...
    categories = [metadata.get('category', 'Unknown') for metadata in documents['metadatas']]  
    ...
    random_colors = [f'#{random.randint(0, 0xFFFFFF):06x}' for _ in range(len(categories))]  
    category_color_map = {category: random_colors[i] for i, category in enumerate(categories)}
    ...
    scatter = go.Scatter(
        ...
        mode='markers',
        marker=dict(size=5, color=colors_for_documents, opacity=0.9),
        text=doc_ids,
        hoverinfo='text'
    )
    ...
    legend_entries = [
        go.Scatter(
            x=[None], y=[None], mode='markers', marker=dict(color=color, size=10),
            name=category  
...
tab1 = gr.Interface(fn=query_rag, inputs="text", outputs="text", title="Query your University Notes")
tab2 = gr.TabbedInterface([
    gr.Interface(fn=visualize_embeddings_interactive, inputs=[], outputs=gr.Plot(), title="Visualize Embeddings"),
    gr.Interface(fn=add_document_interface, inputs="file", outputs="text", title="Add Document"),
    gr.Interface(fn=delete_document_interface, inputs="text", outputs="text", title="Delete Document")
], tab_names=["Embedding Visualization", "Add Document", "Delete Document"])

tabs = gr.TabbedInterface([tab1, tab2], tab_names=["Chat", "Database Management"])
